...
async function sendImportacaoRequest(token, lote) {
    ...
    await axios(configApi).then((response) => {
...
async function executorSendCargaRequest(token, lote, jobs){
    ...
    await Promise.all(jobs.map(job => sendCargaRequest(token, lote, JSON.stringify(job))));
    let responseCarga = await getCargaResponse(token, lote);
    let cargaProcessRunning = responseCarga.filter((response) => response.mensagem.includes('processamento'));
    ...
    while (cargaProcessRunning.length > 0 && cargaTentative <= 30) {
        ...
        cargaProcessRunning = responseCarga.filter((response) => response.mensagem.includes('processamento'));
        await new Promise(resolve => setTimeout(resolve, 5000));
        cargaTentative++;
    }

    let processSucess = responseCarga.filter((response) => response.mensagem.includes('Carga finalizada'));
...
async function executorSendImportacaoRequest(token, lote, process){
    ...
    if (process) {
        ...
        const processSucess = responseImp.filter((response) => response.mensagem.includes('Importação finalizada'));
        console.log(processSucess);
        await exportToFile(path.join(__dirname, 'output','log.txt'), JSON.stringify(processSucess)); 
    }
}
...
async function Execute() {
    ...
    const token = config.ReuseCredentials.token ? config.ReuseCredentials.token : await generateCredentials();
    console.log(token);

    //Chamada para geração do Lote
    const lote = config.ReuseCredentials.lote ? config.ReuseCredentials.lote : await generateLote(token);
    console.log(lote);
